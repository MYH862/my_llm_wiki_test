...
- **BREAKING**: 前端从 Tauri WebView 改为独立的 Web 应用或轻量级桌面客户端（如 Electron 或纯 Web）
- 新增用户认证系统（登录/注册/JWT）
- 新增基于角色的权限控制（RBAC）
...
- **Affected code**: 
  - 后端: `src-tauri/src/` 全部 Rust 代码需要重构为独立 Web 服务
  - 前端: `src/` 中所有调用 Tauri API 的代码需要改为 HTTP API 调用
  - 存储: 从 Tauri Store 改为浏览器 localStorage 或 IndexedDB
  - 部署: 从 Tauri 打包改为 Docker 容器化部署
...
#### 4. 前端改造（高优先级）
- **改造内容**: 移除 Tauri 依赖，改为调用 HTTP API
- **技术方案**: 
  - 选项 A: 纯 Web 应用（React SPA）
  - 选项 B: Electron 桌面应用（保持桌面体验）
  - 选项 C: Tauri 作为纯前端容器（仅保留 WebView 功能）
- **状态管理**: 从 Tauri Store 改为 localStorage/IndexedDB + 服务端同步
...
| 向量数据库 | LanceDB Server | Qdrant, Milvus |
...
| 桌面容器 | Electron | Tauri (仅 WebView), 纯 Web |
...
**Migration**: 前端改为 Electron 或纯 Web 应用，后端改为独立 Web 服务
